jQuery(document).ready(function () {
    var outerwidth = $("#list47").width(); //Asignación de valor de pixelaje de la pantalla
    $("#txt_width").val(outerwidth);
    var res = { '0': 'Sin Informacion' };
    //var Tipo_Pago = { '0': 'Seleccione Opcion', '1': 'CHEQUE', '2': 'TRANSFERENCIA' };

    $("#btn_Reporte").click(function () { //Inicia funcion del boton buscar (btn_Reporte) para cargar el gridview
        //Ocultar botones
        ...
        $.ajax({ //AJAX PARA EJECUTAR SP DE CONSULTA
            //AGREGAR NUEVOS FILTROS
            url: "../Siniestros/OrdenPagoMasivo.ashx?fecha_ini=" + txt_fecha_ini + "&fecha_fin=" + txt_fecha_fin + "&PagarA=" + PagarA + "&Folio_OnBase=" + Folio_OnBase + "&Folio_OnBase_hasta=" + Folio_OnBase_hasta + "&TipoPago=" + TipoPago + "&TipoComprobante=" + TipoComprobante + "&MonedaPago=" + MonedaPago + "&RFC=" + RFC + "&SubSiniestro=" + SubSiniestro + "&TipoPago_PoT=" + TipoPago_PoT + "&Siniestro=" + Siniestro + "&sn_multipago=" + sn_multipago, 
            type: "POST",
            success: function (result) {
                var mydata = $.parseJSON(result);

                if (PagarA == 8) { 
                    LoadGridTerceros(mydata); //Cargar grid Terceros
                }
                else if (PagarA == 7) {
                    LoadGridAsegurados(mydata);//Cargar grid Asegurados
                }
                else {
                    LoadGrid(mydata); //Cargar grid Proveedores
                }
            },
            error: function (err) {
            }
        });
    }); //Fin de la funcion del boton buscar (btn_Reporte) para cargar el gridview
    ...
    function LoadGrid(mydata) {
        ...
        try {
            ...
            var substro = $("#cmbSubsiniestro").val();
            var siniestro = $("[id*=txtSiniestro]").val();            
    ...
    function LoadGridAsegurados(mydata) {
        ...
        try {
            ...
            var substro = $("#cmbSubsiniestro").val();
            var siniestro = $("#txtSiniestro").val();
            var sRFC = $("[id*=txtRFC]").val();
            sRFC = sRFC.replace("&", "!");
